...
def main():
    ...
    print(f"Summary: processed={counters['files']} accepted_files={counters['accepted']} empty/too_small={counters['empty']} errors={counters['errored']} total_chunks={counters['chunks']}")
    if not all_docs:
        print("No documents to ingest after filtering.")
        return

    # batch embed
    batch = args.batch
    for i in tqdm(range(0, len(all_docs), batch), desc="Embeddings"):
        batch_docs = all_docs[i:i+batch]
        vectors = embed_texts([d['text'] for d in batch_docs])
        store.add(batch_docs, vectors)

    print(f"Done. Vector store now holds {store.count()} chunks (added {len(all_docs)}).")
